...
import { Attendance, Course, Holiday, Profile, SpecialWorkingDay, Timetable, UserData } from "./hybrid";
...
export const DATA_NA_WITH_REGNO = 'Data not available for the given registration number';
...
export const PLEASE_TRY_AGAIN_LATER = 'Please try again sometime later';
export const NEITHER_PASSWORD_NOR_DKEY_PROVIDED = 'Neither password nor dkey provided';
...
const isValidDKey = (dKey: string): boolean => {
    ...
    const intersection = new Set([...dKeySet].filter(char => allCharsSet.has(char)));
...
export const decrypt = (encryptedText: string, dKey: string): string => {
    try {
        if (!isValidDKey(dKey)) { throw new Error('Invalid Decryption key'); }
        ...
        throw new Error([FAILED_DECRYPT_C, DKEY_CHECK_C, err.message].join(':::'));
    }
}

export const handleScrapeErrors = async (err: any, regNo: string, password: string): Promise<void> => {
    const [errorType, errorLocation, errorMessage] = err.message.split(":::");
    switch (errorType) {
        case FAILED_DECRYPT_C:
        case FAILED_AUTH_C:
            await OPS.addFailedAuth(regNo, errorLocation + ' ' + errorMessage);
            break;
        case INVALID_AUTH_C:
            await OPS.addInvalidAuth(regNo, password);
            break;
        case FAILED_SCRAPE_C:
            await OPS.addFailedScrape(regNo, errorLocation + ' ' + errorMessage);
            break;
        case PORTAL_ERROR_C:
            await OPS.addPortalError(regNo, errorLocation + ' ' + errorMessage);
            break;
        default:
    }
}
