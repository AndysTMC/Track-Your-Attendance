...
export class FTime {
    ...
    static getSecDiff(time1: FTime, time2: FTime): number {
        const time1InSeconds = time1.hours * 3600 + time1.minutes * 60 + time1.seconds;
        const time2InSeconds = time2.hours * 3600 + time2.minutes * 60 + time2.seconds;
    ...
    toJSON() { { return { hours: this.hours, minutes: this.minutes, seconds: this.seconds } } }
}
...
export class FSMethods {
    ...
    static getTimeLeftString(timeLeft:number) {
        ...
        const formattedTimeLeft = `${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;
...
export const getLiveTime = () => {
    const date = new Date();
    const time = new FTime(date.getHours(), date.getMinutes(), date.getSeconds());
    return time;
}

export const getSessionIndex = (sessions: Array<Session>, sessionMode: string): number => {
    const liveTime = getLiveTime();

    for (let i = 0; i < sessions.length; i++) {
        const session = sessions[i];
        const sessionStartTime = new FTime(session.timing.startTime.hours, session.timing.startTime.minutes, 0);
        const sessionEndTime = new FTime(session.timing.endTime.hours, session.timing.endTime.minutes, 0);

        if (sessionMode === 'ongoing') {
            if (FTime.getSecDiff(sessionStartTime, liveTime) >= 0 && FTime.getSecDiff(liveTime, sessionEndTime) > 0) {
                return i;
            }
...
export const getSessionDetails = (courses: Array<Course>, session: Session): FSession => {
...
export const getPrettierSessions = (sessions: FSession): string => {
    ...
    result += 'Start Time: ' + FSMethods.getDurationString(sessions.startTime, sessions.endTime) + '\n';
...
export const getPrettierAttendanceStats = (attendances: Array<Attendance>): string => {
