...
const getJSESSIONID = async (): Promise<string> => {
	try {
		const response = await fetch(
			"https://parent.srmap.edu.in/srmapparentcorner/LoginPage",
		...
		throw new Error(
			[FAILED_AUTH_C, JSESSIONID_CHECK_C, err.message].join(":::")
		);
	}
};
...
const getCaptcha = async (JSESSIONID: string): Promise<string> => {
	try {
		...
		const response = await axios.post(
			"https://tya-text-extract.onrender.com/extract",
			formData,
			{
				headers: {
		...
		if (text == null || text == undefined)
			throw new Error("Failed to extract text from captcha image");
		return text.trim();
	} catch (err: any) {
		throw new Error(
			[FAILED_AUTH_C, CAPTCHA_CHECK_C, err.message].join(":::")
		);
	}
};
...
const authenticate = async (
	...
	try {
		const response = await fetch(
			"https://parent.srmap.edu.in/srmapparentcorner/Login",
			{
				headers: {
				...
				body: `ccode=${captcha}&txtUserName=${regNo}&txtAuthKey=${encodeURIComponent(
					password
				)}`,
				method: "POST",
			}
		...
		if (responseText.includes("Welcome")) {
			return {
				...
				headers: {
					...headersForGettingPage,
					cookie: `JSESSIONID=${JSESSIONID}`,
				},
			};
		} else if (responseText.includes("Invalid User ID or Password")) {
			throw new Error(
				[INVALID_AUTH_C, CREDENTIALS_CHECK_C, NA_C].join(":::")
			);
		} else {
...
const extractProfilePage = async (
	portal: Portal,
	pageId: number
): Promise<Profile> => {
	try {
		...
		const profileInfo: Profile = {
			...
			semester: getSemesterNumber(
				getTextFromRow("Semester").split(" ")[0]
			),
			program: getTextFromRow("Program / Section").split("/")[0].trim(),
			section: getTextFromRow("Program / Section")
				.split("/")[1]
				.trim()
				.slice(1, -1),
			dob: parseDate(
				getTextFromRow("D.O.B. / Gender").split("/")[0].trim()
			).toLocaleDateString(),
		...
		throw new Error(
			[FAILED_SCRAPE_C, PROFILE_SCRAPE_C, err.message].join(":::")
		);
	}
};
...
const extractTimetablePage = async (
	portal: Portal,
	pageId: number
): Promise<{ timetable: Timetable; courses: Array<Course> }> => {
	try {
		const timetablePage = await getPage(portal, pageId);
		const $ = cheerio.load(timetablePage);
		const courses: Array<Course> = [];
		$("tr")
			.filter((_: any, el: any) => $(el).find("td").length === 5)
			.each((_: any, el: any) => {
				const [code, name, ltpc, faculty, classRoom] = $(el)
					.find("td")
				...
				courses.push({
					...
					ltpc,
					faculty,
					classRoom: extractAndConcatenateRooms(classRoom),
				});
			});
		...
		$("tr")
			.slice(2, 9)
			.each((dayIndex: number, row: any) => {
				$(row)
					.find("td")
					.slice(1)
					.each((sessionIndex: number, sessionEl: any) => {
						const courseCode: string | undefined = courseCodes.find(
							(code) => $(sessionEl).text().trim().includes(code)
						);
						if (courseCode != undefined) {
							const sessionInfo: Session = {
								timing: sessionTimings[sessionIndex],
								courseCode,
							};
							timetable[days[dayIndex] as keyof Timetable].push(
								sessionInfo
							);
						}
					});
		...
		throw new Error(
			[FAILED_SCRAPE_C, TIMETABLE_SCRAPE_C, err.message].join(":::")
		);
	}
};

const extractAttendancePage = async (
	portal: Portal,
	pageId: number
	...
	try {
		...
		throw new Error(
			[FAILED_SCRAPE_C, ATTENDANCE_SCRAPE_C, err.message].join(":::")
		);
	}
};
const setTotalInAttendance = async (
	...
	try {
		const startDateString = systemData.semStartDate;
		const endDateString = systemData.semEndDate;
		...
		const countWeekdayOccurrences = async (
			startDate: Date,
			endDate: Date
			...
			const specialWorkingDays: SpecialWorkingDay[] =
				await OPS.getSpecialWorkingDays();
			...
			specialWorkingDays.forEach((specialWorkingDay) => {
				...
				if (
					specialWorkingDate >= startDate &&
					specialWorkingDate <= endDate
				) {
		...
		const weekdayOccurrences = await countWeekdayOccurrences(
			startDate,
			endDate
		);
		attendanceInfo.forEach((attendance) => {
			...
			days.forEach((dayName, dayIndex) => {
				const sessionsForDay = timetableInfo[
					dayName as keyof Timetable
				].filter((session) => session.courseCode === courseCode);
				totalSessions +=
					sessionsForDay.length * weekdayOccurrences[dayIndex];
			});
...
export const scrape = async (
	...
	try {
		...
		const {
			...
			await extractTimetablePage(portal, 10);
		const attendances: Attendance[] = await extractAttendancePage(
			portal,
			3
		);
		await setTotalInAttendance(timetable, attendances, systemData);
...
export const getPortal = async (
	...
	try {
		...
		const portal: Portal = await authenticate(
			JSESSIONID,
			captcha,
			regNo,
			password
