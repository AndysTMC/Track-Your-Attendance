...
const getUserDataLocal = async (): Promise<UserData | null> => {
    try {
        let userDataLocalString: string | null = localStorage.getItem('TYASRMAPUDATA') ?? null;
        ...
        if (userDataLocalString && regNo) {
            ...
            let response = await axios.post('/api/verify', { regNo, checksum }, {
                headers: {
...
export default function Collect() {
    ...
    const validateUsername = (username: string): boolean => {
        ...
        if (!username.startsWith('AP')) throw new Error('Registration Number should start with AP');
        return true
    }
